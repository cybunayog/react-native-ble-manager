...
public class Peripheral extends BluetoothGattCallback {
	...
	public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
		mainHandler.post(() -> {
			if (!writeDescriptorCallbacks.isEmpty()) {
				if (status == BluetoothGatt.GATT_SUCCESS) {
	...
	public void readDescriptor(UUID serviceUUID, UUID characteristicUUID, UUID descriptorUUID, Callback callback) {
		...
		readDescriptorCallbacks.addLast(callback);
        boolean result = commandQueue.add(new Runnable() {
			@Override
			public void run() {
				if (!gatt.readDescriptor(descriptor)) {
                    for (Callback readDescriptorCallback: readDescriptorCallbacks) {
                        sendBackrError(readDescriptorCallback, "Read failed");
                    }
					readDescriptorCallbacks.clear();
					completedCommand();
				}
	...
	public boolean doDescriptorWrite(final BluetoothGattDescriptor descriptor, byte[] data, final Callback callback) {
		...
		boolean result = commandQueue.add(new Runnable() {
			...
			public void run() {
                ...
                writeDescriptorCallbacks.addLast(callback);
